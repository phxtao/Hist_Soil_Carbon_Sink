function [layer_delta_14c_record, layer_soc_record] = ...
    fun_hist_simu_timestep_site_default(kp, atmo_14c, simu_year_start, simu_year_end, ...
...
default_para_dir = '/Users/ft254/DATAHUB/ENSEMBLE/INPUT_DATA/cesm2_parameters/clm5_params.c190829.nc';
...
fcwdl2 = 1 - ncread(default_para_dir, 'cwd_flig'); % 0.76;w_scaling = 1;
...
matrix_in(1:20,1) = sum(input_vector_cwd, 2)./(days_per_year); % litter input gc/m3/day
matrix_in(21:40,1) = sum(input_vector_litter1, 2)./(days_per_year);
matrix_in(41:60,1) = sum(input_vector_litter2, 2)./(days_per_year);
matrix_in(61:80,1) = sum(input_vector_litter3, 2)./(days_per_year);
...
for iyear = 1:warmup_year_size
    ...
    for itimestep = 1 : timestep_num
        ...
        next_delta_14c_pool(isnan(next_delta_14c_pool) == 1) = -1000;
        %----------------------------------------
        % carbon pool update
        %----------------------------------------
...
layer_delta_14c_record = nan(year_size, n_soil_layer); 
layer_soc_record = nan(year_size, n_soil_layer);
...
if total_c_change_rate > ss_threshold*10 % which means here the threshold is 1gc/m2/yr
    ...
    for iyear = 1 : year_size
        ...
        for itimestep = 1 : timestep_num
            ...
            for ipool = 1:npool
                cpool_12c_stock_annual(itimestep, ipool, :) = next_cpool(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer);
                cpool_14c_stock_annual(itimestep, ipool, :) = next_14c_pool(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer);
                cpool_12c_resp_annual(itimestep, ipool, :) = respired_cpool(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer);
                cpool_14c_resp_annual(itimestep, ipool, :) = respired_14c_pool(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer);
            ...
            if itimestep == timestep_num
                ...
                annual_mean_12c = reshape(sum((median(cpool_12c_stock_annual, 1)), 2), [n_soil_layer, 1]);
                annual_mean_14c = reshape(sum((median(cpool_14c_stock_annual, 1)), 2), [n_soil_layer, 1]);
                annual_mean_delta_14c = (annual_mean_14c./annual_mean_12c - 1)*1000;
                
                layer_delta_14c_record(iyear, :) = annual_mean_delta_14c;
                layer_soc_record(iyear, :) = annual_mean_12c;
            end
