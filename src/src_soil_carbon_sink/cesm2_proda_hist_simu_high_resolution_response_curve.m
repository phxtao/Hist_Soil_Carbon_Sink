...
for icross_valid = cross_valid_id
    ...
    grid_para_result = csvread([data_path, 'output_data/nn_soil_carbon_sink/global_grid_nn_best_seed_', num2str(icross_valid), '_2024-11-09.csv']);
    valid_grid_loc = csvread([data_path, 'output_data/nn_soil_carbon_sink/global_grid_valid_id.csv']);
    ...
    soc_stock_hist_summary = nan(length(valid_grid_loc), length(1:10:111), soil_cpool_num*4, 11, 2); % dim: grid, time, pool and depth
    soc_resp_hist_summary = nan(length(valid_grid_loc), length(1:10:111), soil_cpool_num*4, 11, 2); % dim: grid, time, pool and depth
    soc_tau_hist_summary = nan(length(valid_grid_loc), length(1:10:111), soil_cpool_num*4, 11, 2); % dim: grid, time, pool and depth
    eco_process_hist_summary = nan(length(valid_grid_loc), length(1:10:111), (5*4+1), 11, 2); % dim: grid, time, pool and depth
    ...
    parfor igrid = min(grid_start, length(valid_grid_loc)) : min(grid_end, length(valid_grid_loc))
        ...
        disp([datestr(now,'HH:MM:SS'), ' processing grid ', num2str(igrid)])
        warning off
        ...
        if iscenario == 1
            % altmax current
            cesm2_simu_altmax = repmat(cesm2_simu_altmax_origin(:, 1:ss_forcing_year_num), [1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_altmax = cesm2_simu_altmax(:, 1:year_num);
            % altmax last year
            cesm2_simu_altmax_last_year = repmat(cesm2_simu_altmax_last_year_origin(:, 1:ss_forcing_year_num), [1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_altmax_last_year = cesm2_simu_altmax_last_year(:, 1:year_num);
            % temp
            cesm2_simu_soil_temperature = repmat(cesm2_simu_soil_temperature_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_soil_temperature = cesm2_simu_soil_temperature(:, :, 1:year_num);
            % water
            cesm2_simu_w_scalar = repmat(cesm2_simu_w_scalar_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_w_scalar = cesm2_simu_w_scalar(:, :, 1:year_num);
            % nitrogen
            cesm2_simu_n_scalar = repmat(cesm2_simu_n_scalar_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_n_scalar = cesm2_simu_n_scalar(:, :, 1:year_num);
            % oxygen
            cesm2_simu_o_scalar = repmat(cesm2_simu_o_scalar_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_o_scalar = cesm2_simu_o_scalar(:, :, 1:year_num);
            % input cwd
        ...
        if iscenario == 2
            ...
            % input cwd
            cesm2_simu_input_vector_cwd = repmat(cesm2_simu_input_vector_cwd_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_input_vector_cwd = cesm2_simu_input_vector_cwd(:, :, 1:year_num);
            % input l1
            cesm2_simu_input_vector_litter1 = repmat(cesm2_simu_input_vector_litter1_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_input_vector_litter1 = cesm2_simu_input_vector_litter1(:, :, 1:year_num);
            % input l2
            cesm2_simu_input_vector_litter2 = repmat(cesm2_simu_input_vector_litter2_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_input_vector_litter2 = cesm2_simu_input_vector_litter2(:, :, 1:year_num);
            % input l3
            cesm2_simu_input_vector_litter3 = repmat(cesm2_simu_input_vector_litter3_origin(:, :, 1:ss_forcing_year_num), [1, 1, ceil(year_num/ss_forcing_year_num)]);
            cesm2_simu_input_vector_litter3 = cesm2_simu_input_vector_litter3(:, :, 1:year_num);
        end
        ...
        [cpool_12c_stock_transient, cpool_12c_resp_transient, ~, ~, cpool_tau_transient, eco_process_transient] = ...
            fun_hist_simu_timestep_response_curve(grid_para, atmo_14c, start_year, end_year, grid_npp_allocation, ...
        ...
        var_middle = nan(length(1:10:111), soil_cpool_num*4, 11, 2); % dim: grid, time, pool and depth
        var_middle(1, :, :, :) = mean(cpool_12c_resp_transient(1:20, :, :, :), 1);
        var_middle(2:end, :, :, :) = reshape(mean(reshape(cpool_12c_resp_transient(2:end, :, :, :), [10, length(1:10:111)-1, soil_cpool_num*4, 11, 2]), 1), [length(1:10:111)-1, soil_cpool_num*4, 11, 2]);
        soc_resp_hist_summary(igrid, :, :, :, :) = var_middle;

        var_middle = nan(length(1:10:111), soil_cpool_num*4, 11, 2); % dim: grid, time, pool and depth
        var_middle(1, :, :, :) = mean(cpool_tau_transient(1:20, :, :, :), 1);
        var_middle(2:end, :, :, :) = reshape(mean(reshape(cpool_tau_transient(2:end, :, :, :), [10, length(1:10:111)-1, soil_cpool_num*4, 11, 2]), 1), [length(1:10:111)-1, soil_cpool_num*4, 11, 2]);
        soc_tau_hist_summary(igrid, :, :, :, :) = var_middle;

        var_middle = nan(length(1:10:111), (5*4+1), 11, 2); % dim: grid, time, pool and depth
        var_middle(1, :, :, :) = mean(eco_process_transient(1:20, :, :, :), 1);
        var_middle(2:end, :, :, :) = reshape(mean(reshape(eco_process_transient(2:end, :, :, :), [10, length(1:10:111)-1, (5*4+1), 11, 2]), 1), [length(1:10:111)-1, (5*4+1), 11, 2]);
        eco_process_hist_summary(igrid, :, :, :, :) = var_middle;
    end
    ...
    if is_server == 0
        ...
        save_pathway_soc = [data_path, 'output_data/hist_simu_proda/process_control/cesm2_hist_transient_summary_response_curve_soc_', ...
        ...
        save_pathway_process = [data_path, 'output_data/hist_simu_proda/process_control/cesm2_hist_transient_summary_response_curve_process_', ...
            cesm2_case_name_hat, '_', num2str(start_year), '_',  num2str(end_year), '_cross_valid_', num2str(icross_valid), '_scenario_', num2str(iscenario), '_worker_', num2str(worker_start), '_', num2str(worker_end)];
        save_pathway_resp = [data_path, 'output_data/hist_simu_proda/process_control/cesm2_hist_transient_summary_response_curve_resp_', ...
            cesm2_case_name_hat, '_', num2str(start_year), '_',  num2str(end_year), '_cross_valid_', num2str(icross_valid), '_scenario_', num2str(iscenario), '_worker_', num2str(worker_start), '_', num2str(worker_end)];
        save_pathway_tau = [data_path, 'output_data/hist_simu_proda/process_control/cesm2_hist_transient_summary_response_curve_tau_', ...
            cesm2_case_name_hat, '_', num2str(start_year), '_',  num2str(end_year), '_cross_valid_', num2str(icross_valid), '_scenario_', num2str(iscenario), '_worker_', num2str(worker_start), '_', num2str(worker_end)];
    end

    save([save_pathway_soc, '.mat'], 'soc_stock_hist_summary', '-v7.3');
    save([save_pathway_process, '.mat'], 'eco_process_hist_summary', '-v7.3');
    save([save_pathway_resp, '.mat'], 'soc_resp_hist_summary', '-v7.3');
    save([save_pathway_tau, '.mat'], 'soc_tau_hist_summary', '-v7.3');
end
