...
for iyear = 1:year_size
    for imonth = 1:month_num
        ...
        % Allocation matrix
        a_ma = a_matrix(is_default, fl1s1, fl2s1, fl3s2, fs1s2, fs1s3, fs2s1, fs2s3, fs3s1, fcwdl2, sand_vector(:, imonth, iyear));
        ...
        cpools_capacity = (a_ma*kk_ma-tri_ma)\(-matrix_in);
       	% iteration difference in cpool
		iteration_cpool = (matrix_in + (a_ma*kk_ma-tri_ma)*current_cpool)*days_per_month(imonth);

		% neither residence time nor capacity are supposed to be negative
		negative_value_loc = find(residence_time_cpool < 0 | cpools_capacity < 0);
		cpools_capacity(negative_value_loc) = 0;
		residence_time_cpool(negative_value_loc) = 0;
		iteration_cpool(negative_value_loc) = 0;
		
		% residence time at each soil layer
		...
		%% stepwise iteration
        cpools_next = iteration_cpool + current_cpool; 
