function [cpool_12c_stock_transient, cpool_12c_resp_transient, ...
    ...
    cpool_potential_transient, cpool_capacity_transient, delta_cpool_transient, ...
    cpool_tau_transient, eco_process_transient] = ...
    fun_hist_simu_timestep(kp, atmo_14c, simu_year_start, simu_year_end, vertical_input_product, ...
...
% 12c related var
cpool_12c_stock_transient = nan(year_size, npool*depth_num);
cpool_12c_resp_transient = nan(year_size, npool*depth_num);
% 14c related var
cpool_14c_stock_transient = nan(year_size, npool*depth_num);
coop_14c_resp_transient = nan(year_size, npool*depth_num);
% potential and capacity
cpool_potential_transient = nan(year_size, timestep_num,  npool*depth_num);
cpool_capacity_transient = nan(year_size, timestep_num, npool*depth_num);
% residence time
cpool_tau_transient = nan(year_size, npool*depth_num);
% ecological processes
eco_process_transient = nan(year_size, 5*depth_num+1); % I A K V Xi, NPP
% alteration matrix
alter_matrix_transient = nan(npool*n_soil_layer, npool*n_soil_layer, timestep_num, year_size);
% carbon input vector
input_amount_transient = nan(npool*n_soil_layer, timestep_num, year_size);
input_vector_transient = nan(npool*n_soil_layer, timestep_num, year_size);
% changes of carbon pools
delta_cpool_transient = nan(year_size, timestep_num, npool*depth_num);
...
if total_c_change_rate > ss_threshold*10 % which means here the threshold is 1gc/m2/yr
    ...
    for iyear = 1 : year_size
        ...
        cpool_potential_annual = nan(timestep_num, npool*depth_num);
        cpool_capacity_annual = nan(timestep_num, npool*depth_num);
        ...
        for itimestep = 1 : timestep_num
            ...
            % tri matrix
            tri_ma = tri_matrix(nbedrock_transient(itimestep, iyear), altmax_current_profile_transient(itimestep, iyear), altmax_lastyear_profile_transient(itimestep, iyear), bio, adv, cryo);
            ...
            %----------------------------------------
            % record capacity/potential related
            %----------------------------------------
            alter_matrix_transient(:, :, itimestep, iyear) = a_ma*kk_ma-tri_ma;
            input_vector_transient(:, itimestep, iyear) = input_matrix;
            input_amount_transient(:, itimestep, iyear) = matrix_in;
            ...
            delta_cpool = next_cpool - current_cpool;
            for ipool = 1:npool
                ...
                % cpool_potential_annual(itimestep, ((ipool-1)*depth_num+1):ipool*depth_num) = fun_layer_summary(cpool_potential_season(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer));
                % cpool_capacity_annual(itimestep, ((ipool-1)*depth_num+1):ipool*depth_num) = fun_layer_summary(cpool_capacity_season(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer));
                ...
                % delta_cpool_transient(iyear, itimestep, ((ipool-1)*depth_num+1):ipool*depth_num) = fun_layer_summary(delta_cpool(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer));
            end
            ...
            if itimestep == timestep_num
                ...
                for ipool = 1:npool
                    cpool_tau_transient(iyear, ((ipool-1)*depth_num+1):ipool*depth_num) = fun_layer_summary(tau_annual(((ipool-1)*n_soil_layer+1):ipool*n_soil_layer)); % median(cpool_tau_annual, 1, 'omitnan')';
                end
                % bar(cpool_tau_transient(iyear, :)/365)
                
                % soc potential and capacity
                % cpool_potential_transient(iyear, :, :) = cpool_potential_annual; % mean(cpool_potential_annual, 1, 'omitnan')';
                % cpool_capacity_transient(iyear, :, :) = cpool_capacity_annual; % mean(cpool_capacity_annual, 1, 'omitnan')';
                % cpool_tau_transient(iyear, :) = median(cpool_tau_annual, 1, 'omitnan')';
                % 12c and 14c stock
                cpool_12c_stock_transient(iyear, :) = cpool_12c_stock_annual(4, :);
                cpool_14c_stock_transient(iyear, :) = cpool_14c_stock_annual(4, :);
                %  respiration
                cpool_12c_resp_transient(iyear, :) = sum(cpool_12c_resp_annual, 1, 'omitnan')';
                coop_14c_resp_transient(iyear, :) = sum(cpool_14c_resp_annual, 1, 'omitnan')';
                ...
                eco_process_transient(iyear, :) = [bulk_A', bulk_I, bulk_K, bulk_V, bulk_Xi, sum(carbon_input_annual)];
