function [cpool_ss, soc_layer, is_ss] = ...
    fun_ss_simu(kp, is_default, nbedrock, sand_vector, npp_mean, ...
...
if use_beta == 1
    input_tot_cwd = sum(sum(input_vector_cwd, 2, 'omitnan').*dz(1:n_soil_layer)); % (gc/m2/year)
    input_tot_litter1 = sum(sum(input_vector_litter1, 2, 'omitnan').*dz(1:n_soil_layer));
    input_tot_litter2 = sum(sum(input_vector_litter2, 2, 'omitnan').*dz(1:n_soil_layer));
    input_tot_litter3 = sum(sum(input_vector_litter3, 2, 'omitnan').*dz(1:n_soil_layer));
    ...
    if is_default == 0
        ...
        matrix_in(1:20,1) = sum(input_vector_cwd, 2, 'omitnan')./(days_per_year); % litter input gc/m3/day
        matrix_in(21:40,1) = sum(input_vector_litter1, 2, 'omitnan')./(days_per_year);
        matrix_in(41:60,1) = sum(input_vector_litter2, 2, 'omitnan')./(days_per_year);
        matrix_in(61:80,1) = sum(input_vector_litter3, 2, 'omitnan')./(days_per_year);
        matrix_in(81:140,1) = 0;
    end
    ...
    carbon_input = sum((sum(input_vector_cwd, 2, 'omitnan') ...
        + sum(input_vector_litter1, 2, 'omitnan') ...
        + sum(input_vector_litter2, 2, 'omitnan') ...
        + sum(input_vector_litter3, 2, 'omitnan')).*dz(1:n_soil_layer));
end
...
soc_stock_initial = sum(cpools_layer.*dz(1:n_soil_layer)); % unit gC/m2
%% Post Analytical Spin-up
warmup_year_size = 50000;
ss_forcing_year_num = 1;
circular_year_num = 20;

ss_threshold = 1; % unit 1gc/m2/year
soc_stock_record = soc_stock_initial;
current_cpool = cpools;
is_ss = 0;
% soc_stock_transient = nan(warmup_year_size, 12);
for iyear = 1:warmup_year_size
    year_count = mod(iyear, circular_year_num);
    if year_count == 0
        year_count = ss_forcing_year_num;
    end
    
    if year_count == 1 && iyear ~= 1
        soc_change_rate = abs((soc_stock - soc_stock_record)/circular_year_num);
        if soc_change_rate < ss_threshold % unit 1gc/m2/year
            is_ss = 1;
            break
        else
            soc_stock_record = soc_stock;
        end
    end
    ...
    for imonth = 1 : month_num
        ...
        kk_ma = kk_matrix(monthly_xit, monthly_xiw, monthly_xio, monthly_xin, efolding, tau4cwd, tau4l1, tau4l2, tau4l3, tau4s1, tau4s2, tau4s3);
        % tri matrix
        monthly_nbedrock = nbedrock(imonth);
        monthly_altmax_current_profile = altmax_current_profile(imonth);
        monthly_altmax_lastyear_profile = altmax_lastyear_profile(imonth);
        ...
        if use_beta == 1
            input_tot_cwd = sum(input_vector_cwd(:, imonth).*dz(1:n_soil_layer)); % (gc/m2/month)
            input_tot_litter1 = sum(input_vector_litter1(:, imonth).*dz(1:n_soil_layer));
            input_tot_litter2 = sum(input_vector_litter2(:, imonth).*dz(1:n_soil_layer));
            input_tot_litter3 = sum(input_vector_litter3(:, imonth).*dz(1:n_soil_layer));
            ...
            if is_default == 0
                ...
                matrix_in(1:20,1) = input_vector_cwd(:, imonth)./(days_per_month(imonth)); % litter input gc/m3/day
                matrix_in(21:40,1) = input_vector_litter1(:, imonth)./(days_per_month(imonth));
                matrix_in(41:60,1) = input_vector_litter2(:, imonth)./(days_per_month(imonth));
                matrix_in(61:80,1) = input_vector_litter3(:, imonth)./(days_per_month(imonth));
                matrix_in(81:140,1) = 0;
            end
            carbon_input = input_tot_cwd + input_tot_litter1 + input_tot_litter2 + input_tot_litter3;
        else
            ...
            carbon_input = sum((input_vector_cwd(:, imonth) + input_vector_litter1(:, imonth) + input_vector_litter2(:, imonth) + input_vector_litter3(:, imonth)).*dz(1:n_soil_layer));
        end
        ...
        cpools_layer = sum([current_cpool(1:20), current_cpool(21:40), current_cpool(41:60), current_cpool(61:80), ...
            current_cpool(81:100), current_cpool(101:120), current_cpool(121:140)], 2);
        soc_stock = sum(cpools_layer.*dz(1:n_soil_layer)); % unit gC/m2
        
        soc_layer = sum([current_cpool(81:100), current_cpool(101:120), current_cpool(121:140)], 2); % unit gC/m3
        
        soc_stock_transient(iyear, imonth) = soc_stock;
...
cpool_ss = current_cpool;
soc_layer = sum([cpool_ss(81:100), cpool_ss(101:120), cpool_ss(121:140)], 2); % unit gC/m3
