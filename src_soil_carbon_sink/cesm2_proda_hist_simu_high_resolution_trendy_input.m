...
for icross_valid = cross_valid_id
    ...
    if is_server == 0
        ...
        cd(['/Users/ft254/GitHub/ENSEMBLE/src_soil_carbon_sink/']);
    else
        % server
        data_path = '/GFPS8p/cess11/taof/datahub/ensemble/';
        cd('/GFPS8p/cess11/taof/ensemble/src_soil_carbon_sink/');
    end
    ...
    %     %----------------------tot npp hist change--------------------------%
    %     npp_annual_tot = nan(111, 18);
    %     for imodel = 1 : length(trendy_model_list)
    %         trendy_simu_file = [data_path, 'input_data/trendy_hist/trendy_npp_1900_2010_transient_s_2_',  trendy_model_list{imodel}, '.mat'];
    %
    %         if isfile(trendy_simu_file) == 1
    %             % npp simulated by trendy model
    %             npp_trendy_model = load([data_path, 'input_data/trendy_hist/trendy_npp_1900_2010_transient_s_2_',  trendy_model_list{imodel}, '.mat']);
    %             npp_trendy_model = npp_trendy_model.summary_npp_transient;
    %
    %             for iyear = 1:111
    %                 npp_annual_tot(iyear, imodel) = sum(npp_trendy_model(:, :, iyear).*area_map, 'all', 'omitnan')/10^12;
    %
    %             end
    %         end
    %     end
    %
    %     npp_annual_tot_change = npp_annual_tot - repmat(npp_annual_tot(1, :), [111, 1]);
    %     %----------------------tot npp hist change--------------------------%


    for imodel = trendy_model_num % 1 : length(trendy_model_list)
        % new para informed by new veg and npp info
        grid_para_result = csvread([data_path, 'output_data/nn_soil_carbon_sink/global_grid_nn_best_seed_', num2str(cross_valid_id), '_trendy_', num2str(trendy_model_num), '_2024-11-06.csv']);
        ...
        % for igrid = min(grid_start, length(valid_grid_loc)) : min(grid_end, length(valid_grid_loc))
            disp([datestr(now,'HH:MM:SS'), ' trendy model ', num2str(imodel), ' processing grid ', num2str(igrid)])
            warning off
            ...
            grid_trendy_npp = reshape(npp_trendy_model(grid_lat_coord, grid_lon_coord, :), [size(npp_trendy_model, 3), 1])*1000;
            ...
            % transfer input
            input_transfer_index = grid_trendy_npp./sum(cesm2_simu_npp'.*repmat(days_per_month, [111, 1])*24*3600, 2, 'omitnan');
            input_transfer_index(input_transfer_index < 0 | input_transfer_index > 10) = nan;
            input_transfer_index_smooth = reshape(smooth(input_transfer_index, 10), [1, 1, 111]);
            input_transfer_index_smooth(input_transfer_index_smooth < 0) = 1;

            cesm2_simu_input_vector_cwd_transfer = cesm2_simu_input_vector_cwd_origin.*repmat(input_transfer_index_smooth, [20, 12, 1]);
            cesm2_simu_input_vector_litter1_transfer = cesm2_simu_input_vector_litter1_origin.*repmat(input_transfer_index_smooth, [20, 12, 1]);
            cesm2_simu_input_vector_litter2_transfer = cesm2_simu_input_vector_litter2_origin.*repmat(input_transfer_index_smooth, [20, 12, 1]);
            cesm2_simu_input_vector_litter3_transfer = cesm2_simu_input_vector_litter3_origin.*repmat(input_transfer_index_smooth, [20, 12, 1]);
            ...
            if iscenario == 3
                ...
                % input cwd
                cesm2_simu_input_vector_cwd = cesm2_simu_input_vector_cwd_transfer;
                % input l1
                cesm2_simu_input_vector_litter1 = cesm2_simu_input_vector_litter1_transfer;
                % input l2
                cesm2_simu_input_vector_litter2 = cesm2_simu_input_vector_litter2_transfer;
                % input l3
                cesm2_simu_input_vector_litter3 = cesm2_simu_input_vector_litter3_transfer;
            end

            vertical_input_product = nan(soil_decom_num, 1);
            % fully varying variable

            [cpool_12c_stock_transient, ~, ...
                ~, ~, ...
                ~, ~, ~, ...
                ~, ~] = ...
                fun_hist_simu_timestep(grid_para, atmo_14c, start_year, end_year, vertical_input_product, ...
        ...
        if is_server == 0
            % save_pathway = ['/Users/phoenix/Desktop/cesm2_hist_transient_summary_', ...
            ...
            save_pathway = [data_path, 'output_data/hist_simu_proda/trendy_input/cesm2_hist_transient_summary_trendy_model_', num2str(imodel), '_', ...
                cesm2_case_name_hat, '_', num2str(start_year), '_',  num2str(end_year), '_cross_valid_', num2str(icross_valid), '_scenario_', num2str(iscenario)];
        end

        hist_summary = soc_stock_hist_summary;
